...
public class ImagenesServicioImpl implements ImagenesServicio {
    ...
    public ImagenesServicioImpl(){
        ...
        config.put("cloud_name", "dozktzkwz");
        config.put("api_key", "429381364765633");
        config.put("api_secret", "***************************");
        cloudinary = new Cloudinary(config);
    }

    @Override
    public Map subirImagen(MultipartFile imagen) throws Exception {File file = convertir(imagen);
